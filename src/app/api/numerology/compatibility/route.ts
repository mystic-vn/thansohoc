...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (type === 'life-path-zodiac' && 
      ...
      // Sử dụng cache để lấy dữ liệu từ database
    ...
    return await withCache(
      async () => {
        ...
        if (data) {
          return { data: { ...data, isPersonalCombination: false } };
        }
        
        return { error: 'Không tìm thấy dữ liệu tương hợp', status: 404 };
      },
      'compatibility-api',
      cacheParams,
      7200 // Cache 2 giờ
